...
function createHomeFolder(name) {
    return exec('mkdir -p /var/www/{{name}}', {
            name: name
        })
        .then(function() {
            return exec('chown -R {{name}}:{{name}} /var/www/{{name}}', {
                name: name
            });
        });
}
...
function createBackupFolder(name) {
    return exec('mkdir -p /var/backup-www/{{name}}', {
        name: name
    });
}

function removeHomeFolder(name) {
    return exec('rm -fr /var/www/{{name}}', {
        name: name
    });
}
...
Customer.prototype.remove = function() {
    ...
    .then(function() {
        return exec('deluser --backup --backup-to /var/www/backup --remove-home {{name}}', {
            name: _t.name
        });
    })
